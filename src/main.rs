...
fn main() {
    let store_path = match get_store_path() {
        Ok(s) => s,
        Err(_) => {
            println!("Failed to read store path");
            return;
        }
    };
    println!("store path {:?}", &store_path);
    let session_key = get_session_key_from_user().expect("Failed to get session key");
    ...
    loop {
        ...
        match &tokens[0] as &str {
            ...
            _ => {
                println!("Unknown command. Enter ? for help.");
            }
        }
    }
}
...
fn cmd_push(
    ...
    let secret = if let Ok(mut secret) = get_persistent::<Secret>(session_key, &pb) {
        ...
        if secret.key != key && !user_confirmation(&key_warning(&secret.key)) {
        ...
        Secret {
            key: key.to_owned(),
            value: vec![(Local::now(), value.to_owned())],
            noise: filename
                .as_bytes()
                .iter()
                .cloned()
    ...
    if pb.exists() {
        if let Ok(secret) = get_persistent::<Secret>(session_key, &pb) {
            ...
            if !user_confirmation(
                "Found entry file, but cannot open it using current config. Proceed with rm?",
            ) {
                return;
            }
        };
...
fn write_persistent<T: Serialize>(
    session_key: &SessionKey,
    file_path: &Path,
    t: &T,
) -> Result<(), std::io::Error> {
    ...
    let encrypted_data = encrypt(payload_bytes, &session_key.key, &session_key.iv)
        .ok()
        .unwrap();
...
fn get_persistent<T: DeserializeOwned>(
    session_key: &SessionKey,
    file_path: &Path,
    ...
    if let Ok(decrypted_data) = decrypt(&buf, &session_key.key, &session_key.iv) {
        return deserialize(&decrypted_data).map_err(|_| Error::new(ErrorKind::Other, "bincode"));
    }
